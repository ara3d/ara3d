...
namespace Ara3D.Serialization.G3D
{
    ...
    public struct G3dHeader
    {
        ...
        {
            if (magicA != MagicA) throw new Exception($"First magic number must be {MagicA} not {magicA}");
            if (magicB != MagicB) throw new Exception($"Second magic number must be {MagicB} not {magicB}");
