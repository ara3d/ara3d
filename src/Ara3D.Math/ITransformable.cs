...
namespace Ara3D.Mathematics
{
    ...
    public static class Transformable
    {
        ...
        public static T ScaleX<T>(this T self, float x) where T : ITransformable
            => self.Scale(x, 1, 1);

        public static T ScaleY<T>(this T self, float y) where T : ITransformable
            => self.Scale(1, y, 1);
        ...
        public static T Rotate<T>(this T self, float yaw, float pitch, float roll) where T : ITransformable
            => self.Transform(Matrix4x4.CreateFromYawPitchRoll(yaw, pitch, roll));

        public static T Reflect<T>(this T self, Plane plane) where T : ITransformable
            => self.Transform(Matrix4x4.CreateReflection(plane));

        public static T RotateX<T>(this T self, float angle) where T : ITransformable
            => self.RotateAround(Vector3.UnitX, angle);

        public static T RotateY<T>(this T self, float angle) where T : ITransformable
